        ...
        .then((data: MarketDataResult & { logs?: { id: string; timestamp: string; level: string; message: string; source?: string }[] }) => {
          ...
          if (activePropertyIdRef.current === propertyId) {
            ...
            setResult(prev => {
              ...
              const merged = { ...prev.property, marketData: data };
              // Fetch latest from DB to prevent clobbering other parallel resolves (e.g. schools)
              getProperty(propertyId).then(existingSaved => {
                saveProperty(propertyId, propertyUrl, {
                  property: merged,
                  schools: existingSaved?.data.schools || schoolsData,
                      ...
                      {logs.length === 0 ? (
                        ...
                        <div className="space-y-2">
                          {logs.map((log) => (
                            <div
                              key={log.id}
                              className={`p-2 rounded text-sm font-mono ${log.level === 'error' ? 'bg-red-900/30 text-red-300' :
